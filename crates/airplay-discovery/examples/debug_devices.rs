...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, device) in devices.iter().enumerate() {
        ...
        // Status flags (from raw TXT)
        if let Some(txt) = raw_txt {
            if let Some(flags) = txt.get("flags").or(txt.get("sf")) {
                let flags_val = parse_hex_or_decimal(flags);
                println!("  Status Flags: {} (0x{:X})", flags, flags_val);
                decode_status_flags(flags_val);
                println!();
            }

            if let Some(acl) = txt.get("acl") {
                println!("  Access Control (acl): {}", acl);
                decode_acl(acl);
                println!();
            }
            ...
            if let Some(osvers) = txt.get("osvers") {
                println!("  OS Version: {}", osvers);
            }
        }
        ...
        // Key bits
        println!("  Key feature bits:");
        println!("    Bit  9 (Audio):               {}", device.features.supports_audio());
        println!("    Bit 11 (Redundant Audio):     {}", device.features.supports_redundant_audio());
        println!("    Bit 14 (FairPlay Auth):       {}", device.features.requires_fairplay());
        println!("    Bit 26 (MFi Auth Required):   {}", device.features.requires_mfi());
        println!("    Bit 27 (Legacy Pairing):      {}", features & (1 << 27) != 0);
        println!("    Bit 40 (Buffered Audio):      {}", device.features.supports_buffered_audio());
        println!("    Bit 41 (PTP):                 {}", device.features.supports_ptp());
        println!("    Bit 46 (HomeKit Pairing):     {}", device.features.supports_homekit_pairing());
